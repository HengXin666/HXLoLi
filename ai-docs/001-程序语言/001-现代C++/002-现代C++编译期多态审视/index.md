...
> [!NOTE]
> 如果编译器已经知道你程序里只会出现哪几种类型, 那么每一次调用都穿过一层运行期间接, 究竟是在为谁保留可能性?
> 「C++20 之后可以彻底抛弃 `virtual`」更像一句口号 —— 它成立的前提(类型集合封闭、二进制与源码一起构建)比结论本身重要得多.
> 这篇文章要问的是: 编译期多态把哪些成本挪走了, 又把哪些成本挪了回来.
...
把 `concept`、CTAD、`variant` 和分桶容器放在一起看, 它们其实在回答同一个问题: 一个对象的**类型**什么时候才算真正确定. 运行期多态把答案推到调用发生的那一刻, 编译期多态则要求你在写代码的时候就把答案交出来. 围绕"是否还需要 `virtual`"的争论, 本质上是这个答案的归属权之争.

**事实层面** … 语言演进正在持续扩大"编译期确定"能表达的范围: C++20 带来了 concept 与 `std::variant`, 而 P2300(`std::execution`)、P2900(contracts)与 P2996(静态反射)都已进入 C++26 —— 当反射可用之后, 今天需要手写元编程才能完成的"按类型列表生成分桶存储并自动展开访问", 有相当一部分可以交给编译器生成. 与此同时, `virtual` 在 ABI 边界上的地位并未动摇: 动态库、插件与跨语言脚本层依然依赖它, 而去虚拟化也仍然依赖优化器能看见真实调用目标.

**个人判断** … 接下来被淘汰的不会是 `virtual`, 而是"用 `virtual` 去表达一个本来封闭的类型集合"这种习惯. 当反射与契约把样板成本压下去, 编译期多态的门槛会从"写得出模板"转到"说清类型集合的边界" —— 它会更像一次架构判断, 而不是一次编码技巧. 反过来说, 如果一套范式让可读性和编译时间同时恶化, 而团队里没人读得懂模板报错, 那它只是把技术债从运行期挪到了维护期. 分界线始终是同一句话: 动态性是业务事实, 就保留动态边界; 动态性只是历史写法, 才值得静态化.

## 0x0F 参考来源

- [C++虚函数可以彻底抛弃了? 现代C++编译期多态实战 | CppCon](https://www.bilibili.com/video/BV1gqTs66EpK/) —— 全文观点、示例代码与各时间点讨论的原始出处, 讲者用接口绑定、对象持有、异构容器与设备管理四组例子对照虚函数和现代方案.
- [P2996: Reflection for C++26](https://wg21.link/P2996) —— 静态反射提案, 用于判断"按类型列表批量生成样板"在未来是否仍要手写元编程.
- [P2300: std::execution](https://wg21.link/P2300) —— 标准执行模型提案, 说明调度与组合式异步在标准层面已经离开虚接口范式.
- [P2900: Contracts for C++](https://wg21.link/P2900) —— 契约提案, 与 concept 一起构成"把接口前提前移到声明期与编译期"的另一半.
- [cppreference: std::variant](https://en.cppreference.com/w/cpp/utility/variant) —— 核对 `std::variant` 的封闭集合语义与访问限制.
- [cppreference: Constraints and concepts](https://en.cppreference.com/w/cpp/language/constraints) —— 核对 concept 的约束、包含与重载解析规则.
