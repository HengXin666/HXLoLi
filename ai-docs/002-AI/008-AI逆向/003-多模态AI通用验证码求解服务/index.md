...
原文剖析开源项目 OhMyCaptcha: 用 Playwright + 多模态大模型构建覆盖 19 种验证码类型的自托管求解服务. 它值得被单独拆解的三个理由:
...
> 对比 [001-CF过盾工程](../001-CF过盾工程-从零实现Turnstile绕过/index.md "hxid:hx-43764a8b"): Turnstile 需要穿透 Shadow DOM 点击 + TLS 指纹续命; reCAPTCHA v3 只需要在真实浏览器里调用 `execute()` 拿 token — 印证"不同验证码的检测面不同, 破解复杂度也不同".
...
**事实层面** … 原文已经落到代码的机制包括: YesCaptcha 兼容的异步任务队列、标准策略模式的 Solver 协议、把图片统一缩放到 1440x900 的坐标空间标准化、针对识别与分类分别定制的 System Prompt, 以及本地小模型与云端大模型分工的双后端配置. 这些都在项目里可复现, 不是推测.

**个人判断** … VLM 作为"通用感知接口"降低了写图像处理代码的门槛, 但代价是把不确定性搬进了 prompt 与模型选型. 我倾向于认为, 这类服务的竞争力以后不在"能不能看懂图", 而在两件工程化的事: 一是把输出协议约束到可校验的结构化格式, 让下游不必猜模型的意思; 二是把成本分层当成架构的一部分 —— 高频简单任务留在本地, 低频困难任务才上云 —— 否则调用量一涨, 这条链路会先在账单和延迟上崩掉. 同样的判断也适用于更广的 Agent 场景: 当感知环节由模型承担, 系统的稳定性就取决于接口有多确定, 而不是模型有多聪明.

## 0x05 参考来源

- 原文: [用 AI 从零实现通用验证码求解服务: OhMyCaptcha 原理剖析](https://naxbr.com/article/ohmycaptcha-analysis), nax 的博客, 2026-06-02. 本文的架构描述、代码示例与参数取值均出自这里.
- 姊妹篇: [深入理解 Cloudflare 过盾原理: 无需 AI, 从零实现 CF Turnstile 绕过](../001-CF过盾工程-从零实现Turnstile绕过/index.md "hxid:hx-43764a8b") —— 同系列 001, 讲不需要 AI 的那条路线如何工程化.
- 双模型架构与 [Agent 要懂的 LLM 基础知识](../../001-基础/001-Agent要懂的LLM基础知识/index.md "hxid:hx-ff33eb81") 中"能力分层"的思想一致: 高频低难任务用低成本模型, 低频高难任务用强模型.
