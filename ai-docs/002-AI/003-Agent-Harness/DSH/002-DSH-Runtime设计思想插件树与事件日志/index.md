...
> [!NOTE]
> 一个 agent 最贵的时刻, 不是模型答错, 而是它在崩溃重启后**把已经改过的文件又改了一遍**.
> 要避免这件事, 光有"聊天记录"远远不够: 系统必须能回答三个问题 —— 这个能力是谁给的、这次副作用到底发生了没有、模型究竟看见过什么.
> DeepSeek Harness 把这些归属全部收进两根支柱: **插件树**与**事件日志**. 本文讲的是这套运行时的设计思想, 而不是某个 agent 的用法.
...
> 关联阅读: [DSH Agent Loop 源码剖析](../001-DSH-Agent-Loop源码剖析/index.md "hxid:hx-9eda4e5a") 讲同一套设计在代码里的真实落地 (ReactLoopAgent / inbox / 事件链); 本文讲的是设计思想的整体图景, 两者互为印证.
...
把这两根支柱放到一起看, 它们回答的其实是同一个问题: 当执行者是可以被替换、会被中断、还会自己改文件的模型时, "能力"和"事实"该由谁来记账.

**事实层面** … 这套运行时把"我是谁"交给插件树: 一切皆插件, 能力在加载时进入统一目录、卸载时按相反顺序回收副作用, 每个步骤重新组装一次系统提示词与工具定义; 把"我经历过什么"交给事件日志: 只追加不回改, 模型可见的 surface 从日志重建, 压缩是追加一条替换记录而不是删除旧消息, 崩溃恢复则区分"未开始"与"结果未知"两种状态. 能力接缝把依赖收敛到接口层, 工具调用管线让每次调用都过同一道权限与格式闸门, 边界最终由沙箱和权限系统守住.

**个人判断** … 我倾向于认为, 这会是 agent 基础设施分层的方向: 上层拼能力 (预设、插件、子代理), 下层保事实 (事件日志、恢复语义、审计), 中间靠扩展点而不是靠改 loop 来连接. 反过来说, 只要一个系统还要为"新增一种能力"去改核心循环, 或者还要靠内存里的消息数组解释历史, 它就会在崩溃恢复、回放和多入口一致性上不断付利息. 判断一套 agent runtime 是否值得长期投入, 可以先看它有没有把这两件事彻底分开: 能力的归属可撤销, 事实的归属不可协商.
