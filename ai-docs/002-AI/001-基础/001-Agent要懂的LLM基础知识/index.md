...
> [!NOTE]
> 一个 Agent 聊到第二十轮突然不守规矩, 第一百零一次工具调用吐出坏 JSON —— 第一反应通常是"框架有 bug", 可如果问题根本不在框架里呢?
> 温度、分词、注意力这些看起来离业务很远的东西, 决定了 Agent 的成本、延迟与稳定边界; 把它们当成玄学调参, 就只会在同一个坑里反复摔.
...
把五个角度放在一起看, 它们指向同一个处境: Agent 的不稳定不是某个框架的缺陷, 而是"用概率模型去做确定性工程"这件事本身留下的缝. 分词决定账单, 注意力决定长上下文里哪些约束还活着, 采样决定输出能不能被程序直接消费 —— 这些缝不会因为模型换代而消失, 每代模型只是把它挪到不同的位置.

**事实层面** … 文中的证据都是可核对的性质: 注意力成本随序列长度呈平方增长, Attention Sink 让第一个 token 变成"垃圾桶", Lost in the Middle 让埋在中间位置的约束被稀释, KV cache 随上下文线性增长 (70B 模型、4096 长度时每条序列约 1.3GB). 围绕这些性质出现的工程解法 —— FlashAttention、滑动窗口、PagedAttention、前缀缓存、约束解码 —— 都是在给定性质下做折中, 没有一个把性质本身消除; 约束解码也只保证语法合法, 不保证语义正确.

**个人判断** … 未来被长期固化的可能不是某个具体技巧 (约束解码引擎会换, 某个分页实现会被更细的调度取代), 而是"把模型的输出当成不可信输入来对待"这条工程习惯: 计算交给工具, 结构交给约束解码, 语义校验留在业务侧. 反过来说, 只要验证 Agent 长期稳定性的方式还停留在"聊到第二十轮看它还守不守规矩", 模型升级就只是把同一类问题往后推一点; Agent 的竞争力会更明显地落在这些 LLM 之外的确定性层上.

## 0x05 参考来源

- [《Agent要懂的一些LLM基础知识》 —— 假如今天可以躺平](https://www.bilibili.com/video/BV1M73U6LEM3/) —— 本文五个角度 (分词 / 上下文 / 采样 / 成本延迟 / 能力边界) 的原始出处, 约 33.5 分钟, 0x02 章的关键原话均引自该视频.
- [《智能体AI漫游指南: 从基础到系统》(Haggai Roitman, 中文版 v1.3)](https://github.com/Chasing1020/agentic-ai-guide-zh/releases) —— 视频只覆盖该书前 3 章, 分词膨胀率、Attention Sink、PagedAttention、PBRS 等内容的书面来源; 作者另推荐 AK 老师 3 小时《听懂 LLM》作为入门补充.
