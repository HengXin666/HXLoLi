...
authors: "Heng_Xin"
tags: ["AI Agent", "Agent架构", "上下文工程", "多智能体", "工程实践"]
---

# AI Agent设计原理与工程实践

> [!NOTE]
> 同一个模型, 换一套外壳, 有的 Agent 能可靠跑完一单退款, 有的却在第三步就编出一句"已经帮你处理好了"——差距究竟在模型里, 还是在模型之外?
> 上下文、工具、约束、验证、纠正, 每一环都决定它落地时是助手还是负担; 这篇文章要问的是, 把这些环节拼成生产可用的 Agent, 工程上到底要付出什么.
...
五类工具: **感知 / 执行 / 协作 / 事件触发 / 用户沟通** (后两类名称存疑[不确定]). 其中**执行工具最危险** (错误代价最高), 作者实习时就据此做了高风险拦截.
...
**事实层面** … 文中可核对的现象层证据包括: 上下文窗口被形容为"只有一半的检索引擎"(能捞原始记录, 缺提炼层); harness 的绝大部分代码不是工具本身, 而是流程状态管理、多层压缩、权限分级与容错恢复; 只有改 harness 不换模型的对比里, TerminalBench 2.0 分数从 50+ 到 60+ (视频转述, 未独立复现); 多 Agent 的收益只在"引入了单 Agent 生成时拿不到的新信息"时才成立, 而多 Agent 研究系统的 token 消耗约为普通对话的 15 倍 (视频转述); RVP 的洞察是环境本质上是**不对称验证器** —— 判断坏动作容易, 判断有没有实质进展几乎和解决任务一样难. 这些结论的共同结构是: 收益与成本都落在模型之外, 而且无法靠单次调参抵消.
...
- [《深入理解AI Agent设计原理与工程实践!...》 —— 假如今天可以躺平](https://www.bilibili.com/video/BV1EcGw6eE3o/) —— 本文唯一的内容证据来源, 约 44 分钟, 0x01 章十个主题与 0x02 章关键原话均出自该视频.
- [《智能体AI漫游指南: 从基础到系统》(Haggai Roitman, 中文版 v1.3)](https://github.com/Chasing1020/agentic-ai-guide-zh/releases) —— 与视频所讲的书不是同一本 (章节/作者/概念均不同), 0x04 章为其目录摘要, 用于与视频论点交叉阅读.
