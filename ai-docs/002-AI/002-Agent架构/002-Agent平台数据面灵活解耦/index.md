...
> [!NOTE]
> 如果所有 Agent 都跑在同一套基础设施上, 那么当某个业务要求更高隔离、另一套集群容量见底、某个客户坚持私有网络部署时, 平台还剩多少选择?
> "一个控制面管理多个数据面"听起来只是多了一层抽象, 可它决定的其实是平台能不能跨机器、跨集群、跨云落地.
...
把可扩展、可异构与网络隔离放在一起看, 三者其实在逼同一个答案: Agent 的运行环境不该被平台的部署形态绑定. 当 workspace 从几十个涨到几万个、隔离等级从一种裂成多种、部署位置从公有云延伸到客户私网时, 数据面从控制面里拆出来几乎是唯一出路.

**事实层面** … 文中提到的行业动向是: Anthropic CMA 的 Self-hosted Sandbox 与 Neutree Agent Platform(NAP) 的 BYOI(Bring Your Own Infra) 都把"自带基础设施"作为对外能力, 视频即以这两者为背景讨论数据面解耦. 架构层面的约束同样清楚: workspace runtime 才是真正消耗 CPU、内存与磁盘的部分, 所以它必须独立于控制面扩容; 单一基础设施 (一台物理机、一个 Kubernetes 集群、一个公有云环境) 都有容量边界; 数据面的可达形式不只有 push, 部署在无公网入口的私网中时通信必须改成由数据面主动 pull.

**个人判断** … 解耦之后, 平台的主要压力会从"能不能拆"转向"拆开之后怎么看住": 运行时身份、镜像与 skills 的分发一致性, 以及跨数据面的故障定位, 都会比单数据面时代更难. 如果这些能力不跟上, 多数据面只会把扩展性问题换成运维问题 —— 出故障时没人能快速说清"这次 Agent 行为发生在哪个数据面、用的是哪份配置". 反过来说, 谁先把跨数据面的可观测与调度做成常规能力, 谁才真正兑现了"一个控制面管理多个数据面"的收益.

## 0x09 参考来源

- [《Agent 平台工程实践: 数据面灵活解耦 | 录屏精简版》](https://www.bilibili.com/video/BV1fqN16GEBK/) —— 本文唯一的内容证据来源, 以 Anthropic CMA 的 Self-hosted Sandbox 与 Neutree Agent Platform(NAP) 的 BYOI 为背景, 讲清了控制面与数据面的职责划分、数据面解耦的三个驱动力以及私网场景下的 push / pull 切换.
