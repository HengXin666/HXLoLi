...
> [!NOTE]
> 你的 Agent 记住的应该是什么? 是一堆日志, 还是一条**能从过去通向当下决策的链子**?
> 如果一个用户今天改了偏好、明天打断纠正了你, 你的系统是"秒更新又能回答昨天", 还是两者顾此失彼?
> 日志该追加、知识该分视图、现状该覆盖、历史该双时态、错误该变成 skill —— 而这一切不始于某张精美的架构图, 而始于你决定**哪条信息值得被邀进知识库**.
...
> 关联阅读: [AI Agent 设计原理与工程实践](../../002-Agent架构/001-AI-Agent设计原理与工程实践/index.md "hxid:hx-b764ac0f") 已覆盖"记忆三层框架 / 智能体 RAG / 状态栏"; 本笔记聚焦视频特有的**四层架构 + ledger/views/policy 三件套 + 双时态 + 程序性记忆**, 并把它们落到"日志增量固化"这一具体工程诉求, 两者互补不重叠.
...
对知识库增量沉淀的启示: **写入阶段**要有显式的"值不值得沉淀"信号 (可能靠用户确认/高价值事件触发), **读取阶段**要给 agent 一个显式工具去查而非被动堆上下文. 这正好呼应 [AI Agent 设计原理与工程实践](../../002-Agent架构/001-AI-Agent设计原理与工程实践/index.md "hxid:hx-b764ac0f") 里的"状态栏"和"智能体 RAG"思想.
...
**个人判断** … 我认为这类系统接下来的分水岭不在检索精度, 而在**写入的克制**: 一个什么都想记住的知识库, 会在三个月内退化成没人敢信的数据垃圾场. 真正稀缺的能力是判断"这条信息此刻值不值得固化", 以及在判断错误时能干净地撤销. 另一个尚未被解决的缺口是**语义化遗忘** —— 什么时候一条曾经正确的规则应该过期, 目前仍然只能靠人设阈值, 而这件事在召回类指标上完全看不见.

## 0x04 参考来源

- [面试官问你: Agent 的记忆机制应该怎么设计? (BV1qCEu6zEqs)](https://www.bilibili.com/video/BV1qCEu6zEqs/) —— 本文主线素材; 四层记忆架构、ledger/views/policy 三件套、双时态与程序性记忆的原始论述.

### 关联阅读

- [AI Agent 设计原理与工程实践](../../002-Agent架构/001-AI-Agent设计原理与工程实践/index.md "hxid:hx-b764ac0f") —— 记忆三层框架、智能体 RAG、状态栏、harness 三工序.
- [obsidian-second-brain 全解: 一个会自我重写的 AI 知识库](../003-obsidian-second-brain/index.md "hxid:hx-a953a087") —— "活知识库 / 随输入自我重写 / 矛盾自动调和" 的哲学, 与本文"增量沉淀 + 覆盖 + 快照"是同一意图的两种实现路径.
- [Agent 要懂的 LLM 基础知识](../../001-基础/001-Agent要懂的LLM基础知识/index.md "hxid:hx-ff33eb81") —— RAG 召回"宁少勿多"与上下文工程.
