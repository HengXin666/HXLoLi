...
> [!NOTE]
> 给 Agent 接检索, 最省事的做法永远是"买一个搜索 API 然后填 key". 可一旦把约束换成"尽量完全本地自建"和"凭证配置必须体面", 整个方案的形态就变了 —— 而且变的不只是选型.
> 更反直觉的是: 决定这套栈能不能长期活下来的, 往往不是检索算法, 而是 token 过期那一刻用户要面对多少摩擦.
> 那么, 自建的真实边界在哪里, 凭证配置怎么做才不劝退, 又该拿什么证明它比买 API 更值得?
...
这篇表面上在讲检索, 实际上约束的是一条更普遍的工程线: **一切需要长期凭证的外部能力, 都会在"第一次配置"和"第一次过期"之间失血.** 检索栈只是把这条线暴露得最明显的那类系统 —— 它接的外部件最多, 也就最容易被凭证拖垮.

**事实层面** … 自建的真实边界是分层的: 公开可达的内容能做到完全自建或自建外壳, 平台把持的数据只能做到自建中枢, 剩下的部分无论如何都进不来. 免密钥能力并不是一个全局属性, 而是逐平台成立, 且会随上游改版静默失效 —— 已有实测显示, 同一项目内部声明的可用性与当前真实可用性会发生漂移. 此外, MCP 授权规范明确不为 STDIO 传输设计, 这也解释了本地生态普遍依赖环境变量传凭证的现状.

**个人判断** … 我认为这个领域的竞争点会从"能搜到多少"转向"多久不用管": 元搜索的引擎清单天然会腐烂, 凭证会周期失效, 抓取会被拦 —— 真正决定一套自建栈能否活过半年的, 是它能否把这三类腐烂变成可观测的信号, 而不是等下一个人来重新发现. 更长远一点看, 本地中枢加少量受控出口会成为一种常见形态, 而"完全不依赖外部"多半是一个会在一周内被放弃的目标. 至于评测, 我会把它做成一次可重复的定期运行, 而不是一次性的选型报告 —— 因为在这类系统里, 变化本身才是常态.
...
- [对话记忆与知识库增量沉淀](../../004-记忆/001-对话记忆与知识库增量沉淀/index.md "hxid:hx-462ef6c0") — 知识拿进来之后如何增量维护
- [自维护可插拔记忆层设计](../../004-记忆/002-自维护可插拔记忆层设计/index.md "hxid:hx-37f17262") — 经验如何跨项目复用
