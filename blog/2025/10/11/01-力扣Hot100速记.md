...
- [48. 旋转图像](https://leetcode.cn/problems/rotate-image/)
    ...
    > 数学本质: 两次翻转等于一次旋转
    >    - 竖着看: 第 $j$ 列的东西去了 第 $j$ 行
    >    - 横着看: 第 $i$ 行的东西去了 第 $n - 1 - i$ 列
    >
    > 故, 矩阵按照主对角线翻转, $(i, j) \to (j, i)$
    >
    > 把每一行翻转, $(j, i) \to (j, n - 1 - i)$
...
- [234. 回文链表](https://leetcode.cn/problems/palindrome-linked-list/)
    ...
    > $O(n)$ 时间 + $O(1)$ 空间解法: 查找中间节点 (快慢指针) + 反转链表 后, 双指针
...
- [142. 环形链表 II](https://leetcode.cn/problems/linked-list-cycle-ii/)
    ...
    >
    > 推导: 记 $头 \to 入环点 = a$, $环长 = c$, 那么当慢指针走了 $b$ 步快慢指针 **相遇时候**, 快指针走了 $2b$ 步.
    >
    > 假设 快指针比慢指针多走了 $k$ 圈, 那么此时它们走过 $2b - b = kc$, 即 $b = kc$ 距离.
    >
    > 而慢指针在 **环内** 总共走了: $b - a$, 即 $kc - a$ 到达相遇点.
    >
    > 说明, 从相遇点, 再走 $a$ 步, 就是入环口了, 即 $(kc - a) + a$, 即 $kc$.
    >
    > 为什么呢? 就类似于从入环口跑 $k$ 圈, 最后还是回到入环口.
