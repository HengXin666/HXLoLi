...
笔记的最终呈现对象是**读者**, 不是 AI 也不是作者的审核流程. 写作与收尾必须遵守:

1. **正文只写面向读者的内容.** 生成过程、本地操作 (工具链、脚本命令、目录路径、验证步骤、ASR 转写、provenance、visual-check 等) 一律**不得**出现在正文里.
2. **文首不出现审核提示.** 类似 `[!NOTE] 本文由 AI 辅助沉淀, 需要用户 review` 的对读者无意义的提示, 在成稿中必须移除. 模板中由 makeDoc.py 生成的占位提示只存在于草稿阶段, AI 收尾时删除.
3. **需要用户/作者注意审核的点, 只写两处:**
   - `.hx-mitemite.md` (本目录隐藏文件, 结构化 0x00..begin 的 Q/A block);
   - 与用户的直接对话文本 (简要说明 review 要点).
   绝不把审核事项堆在正文里跳出来给读者看.
4. **`0x03` 章节只承载引用来源.** 只列本文引用过的文章/视频/网页等外部来源: 标题 + 链接 + 一句出处/价值说明. 本地材料路径、转写过程、图像质量校验等一律不进 0x03. 若正文引用了行内链接, 0x03 给出完整出处即可.
5. **成稿检查:** 通读全文, 假装自己是第一次点开这篇文章的读者 — 任何需要内部人才懂的操作细节、任何打断阅读的审核/待办/局限提示, 都要移出正文 (局限与待办属于 `.hx-mitemite.md` 或对话).
...
- 已被代码或文档事实回答的问题, 不写入 `.hx-mitemite.md` 让用户重复回答; 依据只记录在 `.hx-mitemite.md` 或阶段性对话说明中, 不要写进对读者展示的正文.
...
正文必须符合 [HXLoLi-MD 规范](docs/HXLoLi支持的md格式.md) 的标点约定: 使用英文标点并保留 `、` `《》` `“”` `—`. AI 起草/修改完正文后, 用脚本检查是否合规:

```bash
uv run .agents/skills/hx-make-ai-docs/scripts/format_cn_punct.py --check <输出index.md>   # 不合规则 exit 1
uv run .agents/skills/hx-make-ai-docs/scripts/format_cn_punct.py --diff <输出index.md>   # 先看差异
uv run .agents/skills/hx-make-ai-docs/scripts/format_cn_punct.py <输出index.md>          # 原地规范化
```

只有 diff 确认为预期改动后才执行原地改写; 不要把已经符合用户风格的原文硬改.

#### PPT / HTML 演示内嵌

当笔记内容适合做演示页/图表时, 平台用 `#ppt` 链接内嵌同目录 .html 侧车:

- 在同目录放**自包含单文件** `xxx.html` (16:9, 内联样式, 不依赖外部相对资源).
- 在 md 正文写 `[PPT #ppt](xxx.html)`, 可加 `##w100%##` 调宽. 参考真实范例 `react-bits-classified-study`.
- 产出顺序: 先写 index.md 内容, 再把演示页做成侧车 .html, 最后在 md 加 `#ppt` 链接.
- 画**架构图/流程图/时序图/数据流图/状态图**时, 调用 `archify` skill (vendored 于 `.agents/skills/hx-archify`): 写 JSON spec -> `validate` -> `deliver` 出自包含 .html, 再用 `#ppt` 链接内嵌; 它自带动效与交互, iframe 内嵌后原样保留, 不要手绘零散 SVG.
...
**写完整篇笔记后, 必须让它在站点前端可见.** 只写 `ai-docs/.../index.md` 文件是不够的; 侧边栏由脚本从目录结构生成, 不注册就不会出现在侧边栏与搜索里.

完成一篇 ai-docs 笔记 (新建或移动目录) 后, 必须执行:

```bash
node scripts/generateAiDocsSidebar.js   # 重新扫描 ai-docs/ 并重写 sidebarsAiDocs.ts
```

- 该脚本按目录结构生成侧边栏 (自动忽略隐藏文件与私有分类), 支持 `tag.json` 自定义图标/标签.
- 运行后自检: 确认 `sidebarsAiDocs.ts` 中包含新笔记的 id (例如 `知识沉淀/现代C++/日常探索/HXLibs编写串行协程调度器/index`).
- 若只是修改正文内容 (不新增/移动目录), 无需重新生成; 新增、重命名、移动文章目录时必须重新生成.
- 最终回复中必须说明是否执行过该命令; 未执行时必须给出理由.

### 6. 本地开发联动 (VS Code 跳转 + 答题卡)

当用户在本地用 `run.sh` 启动站点时, 平台会同时启动 `scripts/dev-edit-server.mjs` (localhost:3310, 仅本地). 它给 ai-docs 笔记页注入仅本地可见的 UI:

- **本地工具条** (每篇 ai-docs 文章顶部):
  - 「在 VS Code 中打开」: `code -g` 跳到该 `index.md` 的标题行, 并主动把 VS Code 窗口弹到前台 (KDE Wayland 用 KWin 脚本, X11 用 xdotool).
  - **选中正文文字后右键**: 弹出「跳转到 VS Code」菜单, 精确跳到对应源码行和列 (L行:C列). 无选区时放行浏览器原生右键菜单.
  - 改动在 VS Code 里完成后, Docusaurus HMR 即时重渲染.
- **答题卡可折叠区** (`.hx-mitemite.md`, 每篇文章目录最多一个, 隐藏文件):
  - 默认**折叠** (仅显示标题栏: 答题卡 · .hx-mitemite.md + n/m 已答), 点击展开;
  - 展开后每个问题**独立框体** (琥珀色边框圆角卡片, 序号 + 问题 + 答案, 答案未填显示「待填写」).
  - 可点「编辑」切换为原始 textarea 并保存写回本地.
  - 这是审核事项的唯一落盘处 (读者看不到).
- **仅本地渲染**: 组件通过运行时探测 `localhost:3310/health` 决定是否渲染; 生产构建 (`docusaurus build`) 时服务不存在, SSR 输出为 null, 构建产物与页面均不含任何本地编辑 UI.
- 交互路径: 审核要点走答题卡 (点击展开逐个框体阅读), 需要精确改源码时用「选中右键 -> 跳转 VS Code」到对应行列.
