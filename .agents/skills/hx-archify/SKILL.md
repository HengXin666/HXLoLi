...
本 skill 已 vendoring 在 HXLoLi/.agents/skills/hx-archify (顶层 .agents/skills 是软链到同一份). 在 HXLoLi 生态内使用 archify 时, 额外遵守:

- 产出物必须是可以被平台 `#ppt` 侧车机制内嵌的**自包含单文件 HTML**: 同目录放置, 并在 md 里写 `[标题 #ppt](xxx.html)`.
