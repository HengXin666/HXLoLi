...
def find_project_model_config() -> Path | None:
    ...
    while True:
        candidate = current / ".agents" / "skills" / "hx-docs-sediment" / "config.toml"
...
TODO: 只列出本文引用的文章/视频/网页等外部来源 (标题 + 链接 + 一句出处说明). 禁止出现本地文件路径、工具链、生成过程、验证步骤等任何本地操作痕迹.
